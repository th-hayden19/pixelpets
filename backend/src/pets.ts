...
// Sets up Express framework which will simplify creating an HTTP server
const app = express()

const client = mqtt.connect("mqtt://localhost:1883");

// Only when the MQTT client connects, subscribe to (listen for) the given topic pattern (which is the state of each pet ID, shown by wildcard +)
client.on("connect", () => {
